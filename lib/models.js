var Dal = {
  ...
  create: function(obj) {
    ...
    obj = Dal.db.find(obj.model, obj._id);
    Dal.sync(obj);
    return obj;
  },
  ...
  setCurrentUser: function() {
    var user = Dal.db.find("User", $.cookie("user_id"));
...
Dal.db = {
  ...
  find: function(model, id) {
    ...
    if (obj) {
      ...
      if (obj.user_id) { obj.user = Dal.db.find("User", obj.user_id); }

      $.each(Dal.models[obj.model].belongs_to, function(index, value) {
        lowered_value = value.toLowerCase()
        obj[lowered_value] = Dal.db.find(value, obj[lowered_value + "_id"]);
      });
    }
